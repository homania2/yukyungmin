...
- Beans: 스프링이 관리하는 클래스 빈(즈) = @controller를 이용해서 빈으로 반자동등록,
=========HomeController분석 MVC중 컨트롤러단 분석===========
- 클래스특징: 클래스이름은 대문자로 시작하는 규칙.
- 이름의 특징: 낙타등표기법=카멜표기법을 사용. Home+Controller(표기 낙타등 혹처럼)
- 클래스의 MVC종류 역할 표시: 3가지 콤포넌트
...
- @import는 jsp에서 외부 라이브러리(모듈)클래스 가져다가 사용할때 사용
- 개발자가 import 하는 것이 아니고, 이클립스가 자동으로 임포트 시켜준다.
- package: 패키지(묶음, 짐보따리) 자바소스코드(클래스)를 관리하기 위해서 만든 영역. (도메인구조와 유사 http://controller.edu.org <-> org.edu.controller
- org.edu.controller 클래스를 초기에 프로젝트 생성할 때 만들었습니다.
- java폴더클래스루트
- java.폴더안에org폴더안.edu폴더안.controller폴더안쪽에 자바소스코드(자바클래스파일)이 생성된다.

==========스프링 MVC구조 중 View단 home.jsp 화면 분석.(프론트엔드)=============
- 프론트엔드에서 제작한 html디자인파일 개발자가 jsp파일로 프로그램을 입히게 됩니다.
- Java(자바): 오라클에서 제작한(라이센스가 있는) 프로그래밍 언어.
- 자바로 만든 홈페이지가 jsp 이다. MVC중 View단 분석
- jsp(Java Server Page): 스프링 MVC프로젝트 신규생성(새문서 생성)시 아래기본출력
- <% ....%>: jsp파일에서 자바프로그램을 사용할때 사용한다.
- <%@ page 에노테이션 페이지 지시자(어) 역할:예) 페이지 인코딩(한글처리) 등
- <%@ taglib 애노테이션 태그라이브러리 지시자.
- taglib = url("외부자바프로그램용 태그를 가져다가 사용할 주소")
- prefix(접두사=축약어) = "c"
- <%@ page session 세션: 로그인 상태를 유지할때 사용한다.
- 세션: 원래 웹페이지는 페이지 이동시 로그인정보 사라진다. 이런 상황을 방지하기 위해서 세션(변수)을 사용함
- 기본은 session = "true" (세션 변수를 사용하겠다는 의미이고, 기본값)
- 태그: 의복(옷) 살때 가격표 태그와 같은 역할을 하는 html정보 태그이다.
- 가격표태그(정보), 타이틀태그(정보), 바디테그(본문정보), P태그(패러그래프=문단정보)
- <html><head></head><body><p>서버 시간...</p></body></html>
- 이클립스에서 단축키: 저장[Ctrl+S], 뒤로 되돌리기[Ctrl+Z], 앞으로 되돌리기[Ctrl+Y]
- Deploy: 배포
- 새문서 만들기처럼 새 MVC프로젝트 만들때, 기본 생성되는 2개의 파일 
- home.jsp + HomeController.java
- 무료라이센스( 저작권 )
- GNU: GNU 는 Not Unix이다. 유닉스OS 비싸기로 유명.
- LGPL2.0: General Purpose License(일반목적의 라이센스) 공개라이센스
- MIT라이센스: mit공대에서 만든 라이센스. 비영리,영리 맘대로 갖다 쓸수 있는 제일 유연성있는 라이센스.
- 1. 헤로쿠에서는 Hsql 메모리 데이터베이스 사용.(서버를 껐다가 켜면, 내용이 원상복귀)
- 보통 솔루션은 데모페이지가 있습니다. 데모페이지에 자료등록이 가능합니다.
- 보통 데모페이지의 데이터는 서버를 저녁에 껏다가 아침에 다시 켜면, 데모페이지 데이터가 초기화됨.
- Hsql 데이터베이스가 위와 같은 상황에서 사용된다.
- 2. 파스타에서는 Mysql 테이터베이스 사용.(서버를 껐다가 켜도, 내용이 계속 유지됨)
...
- View: home.jsp처럼, 화면부분 
- IDE: 통합개발환경 Integration Development Environment (이클립스, 안드로이드스튜디오, VScode)
- log: 로그 에러상황을 확인할때 보통 많이 사용하는 용어. log4j 사용해서 데이터전송상황, 에러표시 등을 사용하게 됩니다.
- 앞으로 작업순서가 아래처럼 진행됩니다.
- 1. 이클립스에서 학생명의 프로젝트 수정/기능추가
- 2. 이클립스 수정한 결과, Run on Sever 로 톰캣에서 확인OK. http://localhost:8080/
- 3. 깃에 푸시.
- 4. 헤로쿠에서 Deploy Branch(깃브렌치) 클릭 -> 깃소스 -> 클라우드로 배포
- 5. 헤로쿠에서 OpenApp 버튼: https://yukyungmin.herokuapp.com/ (나중에 이력서 포트폴리오 주소가 됩니다.)
#### 20201112(목)
...
- war(와르파일) 웹실행파일, jar(자르파일,Java ARcive파일)
- 깃에 올릴때, 제외시킬 폴더를 지정, target폴더는 깃에서 제외
- .gitignore 파일을 만들어서, target 이름을 적으시면 됩니다.
- POM.xml 파일에서 버전을 변경하게 됩니다.
- root루트 : 최상위 위치를 가리킵니다.
- Program 에 사용되는 Object Module(외부프로그램=모듈) 관리하는 파일을 pom.xml이라고 함.
...
- 인젝트(Inject-주사바늘삽입): 외부클래스 사용할때 쓰는 지시어@Iject 사용
- Aspect(관점): 미술에서 사용하는 용어. @Aspect 사용.
- 지시어 = 에노테이션 = @
- 이클립스 file메뉴 -> Restart 클릭
- 자바버전 1.6 -> 1.8 변경예정.
...
- 파일저장: 화면상단 메뉴아이콘 디스켓모양 클릭 또는 Ctrl + S 저장.
#### 응용SW기초기술활용과목(4개의 단원)
- 4단원: 개발환경 구축하기 시작
